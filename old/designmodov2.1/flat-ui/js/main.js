jQuery(document).ready(function() {
	...
	function init(){
		...
		if (jQuery(document).scrollTop() >= baseSection.top - jQuery(window).height()+500-paddingForMobile){
	...
	function showIconSection(){
		if (showIconSectionRunning == 0){
			...
			jQuery('#icon-heart').animate({top:0, opacity:1,myRotationProperty:1},{
				duration:500,
				step: function(now, tween) {
					if (tween.prop === "myRotationProperty") {
						jQuery(this).css('-webkit-transform','scale('+now+')');
						jQuery(this).css('-moz-transform','scale('+now+')'); 
	...
	jQuery('#responsive').click(function() {
		...
		jQuery('#shade',jQuerysvg).stop().animate({svgOpacity:0},{duration:100,easing:"easeInOutQuad"});
		...
		jQuery('#shade',jQuerysvg).stop().animate({svgTransform:"translate(0)",svgOpacity:0.2, svgWidth:32},{duration:500,easing:"easeInOutQuad"});
	});
	...
	jQuery('#bootstrap').on('mousedown',function(e){
		...
		animateBootstrap = window.setInterval(function(){
			...
			jQuery("#"+lines[getRandomInt(0,11)], jQuerysvg).animate({svgStrokeOpacity: opac[getRandomInt(0,1)]}, {duration: 250, easing: 'easeOutCubic'});
		}, 250);
	...
	jQuery('#fittogether').hover(
		...
		console.log(jQuery('#whitehalf',jQuery(this)).attr('data'));
	...
	jQuery('#freefont').on('click',
		...
		jQuery('#hl',jQuery('#freefont')).stop().clearQueue().animate({svgOpacity:0},250);
		jQuery('#shadow',jQuery('#freefont')).stop().clearQueue().animate({svgOpacity:0},250);
	}).on('mouseup',function(){
		jQuery('#hl',jQuery('#freefont')).stop().clearQueue().animate({svgOpacity:1},250);
		jQuery('#shadow',jQuery('#freefont')).stop().clearQueue().animate({svgOpacity:1},250);
	});
	...
	//LOOP ICON
	window.case = 0;
	jQuery('#loop').hover(function(){
		generateOut = "0,"+getRandomInt(160,290);
		jQuery('#loop-sign',jQuery(this)).stop().clearQueue().animate({svgStrokeDashArray:"0,216"},{duration:1000,easing:"easeInOutQuart",complete:function(){jQuery(this).hide();}});
		jQuery('#shade',jQuery(this)).stop().clearQueue().animate({svgOpacity:0},200);
	},function(){
		jQuery('#loop-sign',jQuery(this)).stop().clearQueue().show().animate({svgStrokeDashArray:"216,0.000001"},{duration:1000,easing:"easeInOutQuart"});
		jQuery('#shade',jQuery(this)).stop().clearQueue().delay(400).animate({svgOpacity:0.15},600);
	});
